# Lo que cambia con respecto a la primera versión son dos cosas:
# 1. La matriz de admitancias tiene en cuenta el estado de las líneas.
# 2. Si la línea está abierta, no se calcula el flujo, ni pérdidas, ni violaciones.

function calcularFlujos(datosLinea::DataFrame,  y_series::Vector{ComplexF64}, y_shunt::Vector{ComplexF64}, 
    U::Vector{ComplexF64}, bMVA::Float64, position_l::Vector{Float64}, log_file::Union{IOStream, Nothing}, log_enabled::Bool)
    ...
    for i in 1:nLineas
        ...
        if position_l[i] >= 0.5  # Solo si la línea está cerrada. Si está abierta, Y=0 para esa línea
    ...
    for i in 1:nLineas
        ...
        if position_l[i] < 0.5
            # Línea desconectada: no hay flujo, pérdidas ni violaciones
            I_lineas[i] = [0.0 + 0.0im, 0.0 + 0.0im]
            S_lineas[i] = [0.0 + 0.0im, 0.0 + 0.0im]
            P_lineas[i] = [0.0, 0.0]
            Q_lineas[i] = [0.0, 0.0]
            P_perdidas[i] = 0.0
            Q_perdidas[i] = 0.0
            log_to_file(log_file, "\nLínea $from-$to abierta: sin flujo ni pérdidas.", log_enabled)
            continue
        end
