...
include("./functions/calcularAdmitancias_lineas.jl")
...
function initialize_log(caso_estudio::String, log_enabled::Bool)
    if log_enabled
        ...
        # Extraer el nombre de la última carpeta del caso_estudio
        subfolder_name = basename(caso_estudio)
        log_filename = "logs/pso_lines_$(subfolder_name)_$(timestamp).log"
...
function runPSO(datos::Tuple, nParticle::Int, nInter::Int, log_enabled::Bool)
    ...
    initialize!(swarm)
    gBest_u, gBest_l, gBest_pg, fitgBest = optimize_swarm!(swarm, log_file, log_enabled) 
...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    # Case folder and data extraction
    case_folder = "../../casos/prueba"
    datos = extraerDatos(case_folder)
    println(datos[1])
    ...
    nInter = 100
    log_enabled = true
    codification = "Cod_Potencia"

    # Run PSO
    mejor_estado, mejor_potencias, mejor_coste = runPSO(
            (datos..., case_folder, codification),  # Añadimos modo_codificacion a la tupla
            nParticle, 
            nInter,
            log_enabled
        )
    println(mejor_estado)
    println(mejor_potencias)
    println(mejor_coste)
end  