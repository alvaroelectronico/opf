...
include("./cargarLibrerias.jl")

function extraerDatos(c::String)
    # Datos de las lineas
    datosLinea = CSV.read("$c/datosLineas.csv", DataFrame)

    # Datos de los generadores
    datosGenerador = CSV.read("$c/datosGeneradores.csv", DataFrame)

    # Datos de la demanda
    datosNodo = CSV.read("$c/datosNodos.csv", DataFrame)
